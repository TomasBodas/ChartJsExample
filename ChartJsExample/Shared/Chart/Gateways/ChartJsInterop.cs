...
namespace Tx
{
    public interface IChartJsInterop
    {
        ...
        Task UpdateLineChart(IEnumerable<Report> reports, DateTime startDate, DateTime endDate);
    }
    ...
    public class ChartJsInterop : IChartJsInterop
    {
        ...
        public async Task Draw(IEnumerable<Report> reports, DateTime startDate, DateTime endDate)
        {
            ...
            await _jsMod!.InvokeVoidAsync("draw", reports, startDate, endDate);
        }

        public async Task UpdateLineChart(IEnumerable<Report> reports, DateTime startDate, DateTime endDate)
        {
            if (!await LoadModules()) { return; }
            await _jsMod!.InvokeVoidAsync("update", reports, startDate, endDate);
        }
    }
}